...
public class CheckoutService {
    ...
    for (CheckoutItemRequest item : checkoutRequest.getItems()) {
        ...
        long available = product.getQuantity() == null ? 0L : product.getQuantity();
        if (available <= 0) {
            throw new RuntimeException("The product is sold out.");
        }
        if (available < item.getQuantity()) {
            throw new RuntimeException("Insufficient stock for product: " + product.getName() + ". Available: " + available);
        }

        // decrement stock and save product
        long remaining = available - item.getQuantity();
    ...
    // Clear the cart only if the request marks checkout as successful
    Boolean successFlag = checkoutRequest.getSuccess();
    boolean successful = successFlag != null ? successFlag : true; // default: true
    if (successful) {
